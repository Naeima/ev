# dashboard_ev_merged_rcsp_anim_weather.py
# EV chargers + flood overlays + RCSP (exact/OSRM fallback) + car animation
# + Weather/Air/NRW cards (Met Office optional; Open-Meteo fallback)
...
def compute_at_risk(ev_gdf, max_m):
    ...
    if water.empty:
        out['DistWater_m']=float('nan'); out['AtRisk']=pd.NA; return out
    try:
...
def rcsp_optimize(start_lat, start_lon, end_lat, end_lon,
    ...
    _ = ox.project_graph(G, to_crs="EPSG:27700")  # force CRS attrs on edges
    ...
    if "length" not in edges.columns:
        # fallback approximate length in meters
...
# =========================
# Folium rendering (with animation) + color/legend fixes
# =========================
...
def charger_icon_color(status_label: str, at_risk):
    if at_risk is pd.NA or at_risk is None:
        return 'lightgray'
    if status_label == 'Operational':
        return 'orange' if bool(at_risk) else 'green'
    if status_label == 'Not operational':
        return 'red' if bool(at_risk) else 'gray'
    return 'lightgray'
...
(function() {{
  ...
  var map = null; for (var k in window) {{ if (k.startsWith('map_')) {{ map = window[k]; break; }} }}
  if (!map) return;
  var carIcon = L.divIcon({{ html: '<div style="font-size:20px;transform:translate(-8px,-8px);">🚗</div>', iconSize: [16,16], className: 'car-icon'}});
  var poly = L.polyline(coords, {{color:'#3388ff', weight:4, opacity:0.0}}).addTo(map);
  try {{ map.fitBounds(poly.getBounds(), {{padding:[20,20]}}); }} catch(e){{}}
  var marker = L.marker(coords[0], {{icon: carIcon}}).addTo(map);
  function toRad(d) {{ return d * Math.PI/180; }}
  function hav(a,b) {{ var R=6371008.8; var dLat = toRad(b[0]-a[0]), dLon = toRad(b[1]-a[1]); var la1 = toRad(a[0]), la2 = toRad(b[0]); var h = Math.sin(dLat/2)**2 + Math.cos(la1)*Math.cos(la2)*Math.sin(dLon/2)**2; return 2*R*Math.asin(Math.sqrt(h)); }}
  var seg = [], total=0; for (var i=0;i<coords.length-1;i++) {{ var d = hav(coords[i], coords[i+1]); seg.push(d); total += d; }}
  if (total <= 0) return; var speed = Math.max(1, {float(speed_kmh)}) * 1000.0/3600.0;
  var t0 = performance.now(); var handle = null;
  function step(ts) {{ var dt = (ts - t0)/1000.0; var dist = Math.min(total, dt*speed); var run=0, idx=0; while (idx < seg.length && run + seg[idx] < dist) {{ run += seg[idx]; idx++; }} var p=0; if (idx < seg.length && seg[idx] > 0) {{ p = (dist - run)/seg[idx]; }} else {{ idx = seg.length-1; p = 1; }} var a = coords[idx], b = coords[Math.min(idx+1, coords.length-1)]; var ll = [a[0] + (b[0]-a[0])*p, a[1] + (b[1]-a[1])*p]; marker.setLatLng(ll); if (dist >= total - 1) {{ cancelAnimationFrame(handle); window._ev_anim.active=false; return; }} handle = requestAnimationFrame(step); window._ev_anim.handle = handle; }}
  window._ev_anim.active = true; window._ev_anim.stop = function() {{ try {{ cancelAnimationFrame(handle); }} catch(e){{}} window._ev_anim.active=false; }}; handle = requestAnimationFrame(step);
}})();
</script>
""")
...
def render_map_html_route(full_line, route_safe, route_risk, start, end, chargers,
    ...
    for st in chargers:
        ...
        color = charger_icon_color('Operational' if st.get('Operational') else 'Not operational', st.get('AtRisk'))
        popup = folium.Popup(
    ...
    # Legend for charger status/risk
    legend_html = f"""
...
def render_map_html_ev(df_map, show_fraw, show_fmfp, show_live, show_ctx, near_threshold):
    ...
    for _, row in df_map.iterrows():
        ...
        near = row.get("AtRisk", pd.NA)
        dist_m = row.get("DistWater_m")
        parts = [f"<b>Operator:</b> {row.get('Operator','EV')}", f"<b>Status:</b> {status}"]
        if pd.notna(dist_m): parts.append(f"<b>Dist. to water:</b> {float(dist_m):.0f} m")
        if pd.notna(row.get('dateCreated', pd.NaT)): parts.append(f"<b>Installed:</b> {row['dateCreated']}")
        color = charger_icon_color(status, near)
        folium.Marker([row['Latitude'], row['Longitude']],
                      tooltip=f"{row.get('Operator','')} ({row.get('Town','')})",
...
METOFFICE_SITE_API = os.environ.get("METOFFICE_SITE_API", "").strip()  # e.g., https://datahub.metoffice.gov.uk/forecast/site-specific/point
...
app.layout = html.Div([
    html.H1("South Wales EV – Chargers, Flood Risk, Graph/DP, Animation & Weather"),
...
def _update_map(towns, town_like, op_vals, risk_vals, near_m, layers_vals, risk_json, _tok,
    ...
    if risk_vals and len(risk_vals) < 3:
        if 'AtRisk' in d.columns:
            mask = pd.Series(False, index=d.index)
            if "at" in risk_vals: mask |= d['AtRisk'].eq(True)
            if "no" in risk_vals: mask |= d['AtRisk'].eq(False)
...
def _wx_split(loc, _n):
    ...
    right_children = [html.H3("Next 24h forecast")]
    try:
    ...
    right_children += [dcc.Graph(figure=fig, config={"displayModeBar": False})]
    right = html.Div(style={"border":"1px solid #eee","borderRadius":"10px","padding":"10px"}, children=right_children)
